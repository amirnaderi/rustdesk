...
fn get_after_install(exe: &str) -> String {
    ...
    // https://github.com/dshldesk/dshldesk/commit/f4bdfb6936ae4804fc8ab1cf560db192622ad01a
...
copy /Y \"{tmp_path}\\Uninstall {app_name}.lnk\" \"{start_menu}\\\"
    ...
    // Note: without if exist, the bat may exit in advance on some Windows7 https://github.com/dshldesk/dshldesk/issues/895
    let dels = format!(
        "
...
fn run_after_run_cmds(silent: bool) {
    ...
    if !silent {
        ...
        allow_err!(std::process::Command::new("cmd")
            .arg("/c")
            .arg("timeout /t 2 & start dshldesk://")
