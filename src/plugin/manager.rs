...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    vec![PluginSource {
        name: "dshldesk".to_string(),
        url: "https://raw.githubusercontent.com/fufesou/dshldesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "DshlDesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "DshlDesk wants to uninstall the plugin",
    )
}
