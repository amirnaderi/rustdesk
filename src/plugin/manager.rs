...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    vec![PluginSource {
        name: "dshelpdesk".to_string(),
        url: "https://raw.githubusercontent.com/fufesou/dshelpdesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "DsHelpDesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "DsHelpDesk wants to uninstall the plugin",
    )
}
