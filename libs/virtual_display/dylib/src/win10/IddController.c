...
char g_lastMsg[1024];
const char* g_msgHeader = "DsHelpDeskIdd: ";
...
BOOL InstallUpdate(LPCWSTR fullInfPath, PBOOL rebootRequired)
{
    ...
    if (FALSE == UpdateDriverForPlugAndPlayDevicesW(
        NULL,
        L"DsHelpDeskIddDriver",    // match hardware id in the inf file
        fullInfPath,
        INSTALLFLAG_FORCE
...
BOOL DeviceCreate(PHSWDEVICE hSwDevice)
{
    ...
    PCWSTR description = L"DsHelpDesk Idd Driver";

    // These match the Pnp id's in the inf file so OS will load the driver when the device is created
    PCWSTR instanceId = L"DsHelpDeskIddDriver";
    PCWSTR hardwareIds = L"DsHelpDeskIddDriver\0\0";
    PCWSTR compatibleIds = L"DsHelpDeskIddDriver\0\0";
    ...
    // Create the device
    HRESULT hr = SwDeviceCreate(L"DsHelpDeskIddDriver",
        L"HTREE\\ROOT\\0",
        &createInfo,
        0,
        NULL,
