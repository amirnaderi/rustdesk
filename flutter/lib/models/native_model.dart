...
class PlatformFFI {
  ...
  DshelpdeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = Platform.isAndroid
        ? DynamicLibrary.open('libdshelpdesk.so')
        : Platform.isLinux
            ? DynamicLibrary.open('libdshelpdesk.so')
            : Platform.isWindows
                ? DynamicLibrary.open('libdshelpdesk.dll')
                : Platform.isMacOS
                    ? DynamicLibrary.open("liblibdshelpdesk.dylib")
  ...
  void _startListenEvent(DshelpdeskImpl dshelpdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = dshelpdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
