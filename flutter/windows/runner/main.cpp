...
typedef char** (*FUNC_DSHLDESK_CORE_MAIN)(int*);
typedef void (*FUNC_DSHLDESK_FREE_ARGS)( char**, int);
const char* uniLinksPrefix = "dshldesk://";
...
{
  HINSTANCE hInstance = LoadLibraryA("libdshldesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libdshldesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_DSHLDESK_CORE_MAIN dshldesk_core_main =
      (FUNC_DSHLDESK_CORE_MAIN)GetProcAddress(hInstance, "dshldesk_core_main_args");
  if (!dshldesk_core_main)
  {
    std::cout << "Failed to get dshldesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_DSHLDESK_FREE_ARGS free_c_args =
      (FUNC_DSHLDESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = dshldesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "Dshldesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  ...
  // Uri links dispatch
  HWND hwnd = ::FindWindow(_T("FLUTTER_RUNNER_WIN32_WINDOW"), _T("DshlDesk"));
  if (hwnd != NULL) {
  ...
  if (!window.CreateAndShow(
          is_cm_page ? L"DshlDesk - Connection Manager" : L"DshlDesk", origin,
          size, !is_cm_page)) {
      return EXIT_FAILURE;
  }
