...
const char* uniLinksPrefix = "dshelpdesk://";
...
{
  HINSTANCE hInstance = LoadLibraryA("libdshelpdesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libdshelpdesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_RUSTDESK_CORE_MAIN dshelpdesk_core_main =
      (FUNC_RUSTDESK_CORE_MAIN)GetProcAddress(hInstance, "dshelpdesk_core_main_args");
  if (!dshelpdesk_core_main)
  {
    std::cout << "Failed to get dshelpdesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  int args_len = 0;
  char** c_args = dshelpdesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
  ...
  // Uri links dispatch
  HWND hwnd = ::FindWindow(_T("FLUTTER_RUNNER_WIN32_WINDOW"), _T("DsHelpDesk"));
  if (hwnd != NULL) {
  ...
  if (!window.CreateAndShow(
          is_cm_page ? L"DsHelpDesk - Connection Manager" : L"DsHelpDesk", origin,
          size, !is_cm_page)) {
      return EXIT_FAILURE;
  }
